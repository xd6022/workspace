...
- **过度抽象风险**：简单场景用 LangGraph 杀鸡用牛刀，[[LangChain]] 的 `create_agent` 更合适
...
| [[Deep Agents]] | 基于 LangGraph 的高层 Agent SDK，提供规划、子 Agent、文件系统 |
| [[LangSmith]] | 配套的可观测性和部署平台 |
| **Pregel（Google）** | LangGraph 图执行模型的原型，Google 的大规模图处理系统 |
| **Apache Beam** | 另一个灵感来源，统一批流处理框架 |
| [[CrewAI]] / [[AutoGen]] | 竞品多 Agent 框架，偏高层抽象；LangGraph 偏底层 |
| **DAG（有向无环图）** | 基础图论概念，LangGraph 支持有环图（循环 Agent 调用） |
| **Finite State Machine** | 概念相近，但 LangGraph 的 State 更丰富（任意复杂数据结构） |
...
1. **LangGraph 的并行执行机制**：同一 super-step 内的多个节点如何并行？底层用了什么并发模型（asyncio / 线程池）？
2. **Functional API vs Graph API 的性能对比**：两种 API 在执行效率和内存占用上有差异吗？
3. **Checkpointer 的后端选择**：MemorySaver vs PostgresSaver vs SqliteSaver 的适用场景和性能差异？
4. **Streaming 机制的实现细节**：支持哪些流模式（token-level / node-level / custom）？和普通 LangChain streaming 有何不同？
5. **多 Agent 通信模式**：子图（subgraph） vs 直接节点间消息传递？各自适用场景？
6. **TypeScript 版本的成熟度**：LangGraph.js 和 Python 版本的功能差距具体在哪？
7. **与 Deep Agents 的边界**：[[Deep Agents]] 在 LangGraph 之上到底封装了什么？什么场景应该从 LangGraph 迁移到 [[Deep Agents]]？

---

*关联笔记：[[LangChain]] · [[LangSmith]] · [[Deep Agents]] · [[CrewAI]] · [[AutoGen]]*
